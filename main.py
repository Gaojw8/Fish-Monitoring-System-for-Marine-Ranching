...
# if os.path.exists(f'./ocean_observation_data.csv'):
#     pass
# else:
#     with open(r'./ocean_observation_data.csv','w'):
#         pass
    ...
    def run(self):
        """
           ...
           """
        while True:
            df = pd.read_csv("ocean_observation_data.csv")
            temprature = df["Temperature"]
            salinity = df["Salinity"]
            oxygen = df["Dissolved Oxygen"]
            density = df["Density"]
            tim = df["time"]
            time.sleep(1)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.pushButton_forcast.clicked.connect(self.button_forcast)
        ...
        self.button_jiance.clicked.connect(self.run_jiance)  # 暂停/开始
        self.button_qingchu.clicked.connect(self.qingchu)  # 终止
    ...
    def button_forcast(self):  # 触发button_forcast后的事件
        ...
        self.seriesK = QLineSeries()
        self.seriesS.setName("温度")
        self.seriesL.setName("盐度")
        self.seriesK.setName("溶解氧")
    def qingchu(self):
        self.seriesS.clear()
        self.seriesL.clear()
        self.seriesK.clear()
        self.chart.addSeries(self.seriesS)
        self.chart.addSeries(self.seriesK)
    ...
    def open_src_file(self):
        ...
        if self.task == "Track":
            self.show_status("目前页面：image or video检测页面，Mode：Track")
        if self.task == "forcast":
            self.show_status("目前页面：image or video检测页面，Mode：forcast")
    ...
    def run_jiance(self):
        ...
        self.thread1.finishSignal.connect(self.data_get)
        self.thread1.start()
    def data_get(self,str_):
        df = pd.read_csv("ocean_observation_data.csv")
        temprature = df["Temperature"].astype(float)
        salinity = df["Salinity"].astype((float))
        oxygen = df["Dissolved Oxygen"].astype(float)
        density = df["Density"]
        tim = df["time"].astype((float))
        for i in range(0,100):
            self.seriesS.append(tim[i], temprature[i])
            self.seriesL.append(tim[i], salinity[i])
            self.seriesK.append(tim[i], oxygen[i])
        self.chart = QChart()
        self.chart.setTitle("牧场实时状况")
        self.chart.addSeries(self.seriesS)
        self.chart.addSeries(self.seriesL)
        self.chart.addSeries(self.seriesK)
